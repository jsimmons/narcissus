...
pub fn sin_cos_pi_f32(a: f32) -> (f32, f32) {
    ...
    let sign_c = f32::from_bits((i & 2) << 30);
    let sign_s = f32::from_bits((i << 31) ^ ((i & 2) << 30));
    ...
    // Reconstruction.
    let c = c.copysign(sign_c);
    let s = s.copysign(sign_s);

    // Branchless swap.
    let t = c;
    let c = select_f32(c, s, i & 1 != 0);
    let s = select_f32(s, t, i & 1 != 0);
